...
<meta name="description"
  content="Основные команды linux терминала, необходимые пользователю">
</head>
...
<body>
  ...
  <section>
    <h1 id="top">ОСНОВНЫЕ КОМАНДЫ LINUX ТЕРМИНАЛА</h1>
    ...
    <p>
      ...
      терминала, которые я использую чаще всего.
    </p>
    ...
    <figure>
      <img alt="Консоль vps сервера"
        src="/images/arenda-vps/picture13.png" class="large">
    </figure>
    ...
    <aside class="article-preview">
      ...
      <p>
        Краткий гайд по подключению по SSH-клиентом к виртуальному
        серверу (VPS)... <a href="/blog/podklyuchenie-po-ssh-k-vps.jsp"><b>читать</b></a>
      </p>
    </aside>

    <p>В любом случае будет полезно знать несколько команд linux
      терминала.</p>
  </section>
...
7 - содержательный параметр (показать содержимое src)
  ...
  <section>
    ...
    <p>Обновляет указанный пакет. Без параметров обновляет все
      установленные пакеты.</p>

    <div class="vimcode">apt install [package]</div>

    <p>
      Программа <b>apt</b> ищет указанный пакет, читает служебную
      информацию о нем, при необходимости решает зависимости (т.е.
      устанавливает все другие пакеты, необходимые для работы программ,
      устанавливаемого пакета), а затем устанавливает сам пакет.
    </p>
  </section>
  ...
  <section>
    ...
    <div class="vimcode">rm [file]</div>

    <p>
      Удалить <b>file</b>.
    </p>

    <div class="vimcode">rm -r dir</div>

    <p>Удалить директорию и все ее содержимое. Проходит по
      директории рекурсивно и удаляет все файлы (в linux директории тоже
      являются файлами).</p>

    <div class="vimcode">ls</div>

    <p>Показать список файлов и каталогов (содержимое текущей
      территории).</p>
...
.  ..  ico  images  index.jsp  style.css  .settings
    ...
    <p>
      Вывести скрытые файлы (<b>.settings</b>).
    </p>
...
drwxrwxr-x 2 root root 4096 фев 28 21:14 .settings
    ...
    <p>Короткие ключи можно "склеивать" (команда выведет как
...
# ls --color=none   параметрический
    ...
    <p>
      Ключи могут быть обычные и параметрические. К последним можно
      передать параметры (<b>--color=none</b> убрать раскраску).
    </p>

    <div class="vimcode">pwd</div>

    <p>Возвращает полный путь к тому месту, где мы находимся. Полный
...
# cd ./bin  относительный
    ...
    <p>
      ...
      существует, программа попросит разрешения на его перезапись.
    </p>

    <div class="vimcode">.</div>
    ...
    <div class="vimcode">history</div>

    <p>
      Выводит историю введенных пользователем команд. Как правило,
      пользуются стрелками <b>вверх</b> и <b>вниз</b>, чтобы не вводить
      длинную команду заново, а просто вызвать ее из истории.
    </p>
  </section>
  ...
  <section>
    ...
    <div class="vimcode">touch [file]</div>

    <p>
      Создать файл с именем <b>file</b>. Если такой файл уже существует,
      то изменяет дату его создания.
    </p>

    <div class="vimcode">
      cat [file]</br> cat [file1] >> [file2]
    </div>

    <p>
      Вывести содержимое файла с именем <b>file</b> в стандартный поток
      вывода (на экран). Прочитать <b>file1</b> и дописать его
      содержимое в конец <b>file2</b>.
    </p>

    <div class="vimcode">less [big-file]</div>

    <p>Прочитать файл. Используется для постраничного вывода текста.</p>

    <div class="vimcode">tail [file]</div>
...
# tail -f /var/log/auth.log
    ...
    <p>
      Выведет последние 10 строк файла, при этом файл не закрывается и
      обновляет информацию, показывая изменения в нем.</b>
    <div class="vimcode">
      head [file1]</br> head -3 [file2]
    </div>

    <p>
      Выведет первые 10 строк <b>file1</b> (по умолчанию) и первые три
      строки <b>file2</b>.
    <div class="vimcode">find</div>

    <p>Команда для поиска файлов в linux. Имеет много параметров и
...
# find src -name *.java
    ...
    <p>
      Найти в директории <b>src</b> все <b>java</b>-файлы.
    </p>
...
# find . -name *.tmp -ok rm {} \;
    ...
    <div class="vimcode">echo</div>

    <p>
      Команда <b>echo</b> выводит значение переданного ей аргумента на
      экран.
    </p>
    ...
    <p>
      ...
      список директорий, в которых будет производиться поиск бинарного
      файла для его выполнению по умолчанию (все команды, такие как <b>ls,
        cd, echo</b> являются скомпилированными программами, которые
...
# echo "one parameter"
    ...
    <p>
      Параметры, принимаемые командой, разделяются пробелами (<b>first
        second</b>). С помощью "кавычек" можно передать несколько слов как
      один параметр (<b>"one parameter"</b>).
    </p>

    <div class="vimcode">vim [file]</div>

    <p>
      Открыть файл <b>file</b> в текстовом редакторе <b>vim</b>, либо
  ...
  <section>
    ...
    <div class="vimcode">> (redirection)</div>
...
# cat file.txt
    ...
    <p>
      Записать ( <b>></b> ) текущую дату (<b>date</b>) в файл <b>vile.txt</b>
      (создать его при необходимости), прочитать (<b>cat</b>) содержимое
      файла и вывести на экран (стандартный поток вывода). При
      перенаправлении с помощью ( <b>></b> ) предыдущие данные в файле <b>file.txt</b>
      будут стерты.
    </p>
...
2: stderr   вывод ошибок        в консоль)
    ...
    <p>
      Перенаправление ( <b>></b> ) по умолчанию работает с первым
      потоком (<b> > </b> и <b> 1> </b> одно и то же).
    </p>

    <div class="vimcode">>> (appending)</div>

    <p>
      Одинарное перенаправление ( <b>></b> ) перезаписывает файл,
      двойное ( <b>>></b> ) - дописывает в конец.
    </p>
...
src/main/ROOT/WEB-INF/classes/DevopsServlet.java
    ...
    <p>
      Например, с помощью команды <b>find</b> ищем все файлы в
      директории <b>src</b> и не выводим их на экран, а передаем в
      программу <b>grep</b>. Уже с ее помощью выводим на экран только <b>java</b>
      файлы.
    </p>
  </section>
...
lrwxrwxrwx 2 root root 1096 Mar 25 22:43 link       (ссылка)
    ...
    <p>Последующие буквы разделены на три части по три символа и
      означают права доступа:</p>
...
-rwxr-xr-x 1 root root 1150 Mar 25 22:43 file.class
    ...
    <p>
      Меняем владельца файла с <b>user</b> на <b>root</b>.
    </p>

    <div class="vimcode">chmod</div>
...
-rwxrw-r--  ... file.class              символы
 ...
 |<-          111 = 4+2+1 = 7
    ...
    <p>
      Символы (<b>-rwxrw-r--</b>) на самом деле обозначают биты (<b>111110100</b>).
      Три группы по три бита можно преобразовать в трехзначное число (<b>764</b>),
      которым обозначают права в числовом виде (абсолютный способ
      укащания прав доступа).
    </p>

    <p>Наиболее популярные права доступа:
    <ul>
      <li><b>644</b> - владельцу можно читать и изменять файл,
        остальным пользователям - только читать</li>
      <li><b>666</b> - читать и изменять файл можно всем
        пользователям</li>
