...
<body>
  ...
  <section>
    ...
    <p>
      Версионирование программы внедряется тогда, когда у программы появляется публичный интерфейс, доступный пользователю (<b>API</b>).
    </p>
...
|<----- Major
    ...
    <p><b>Major</b> версия (первая цифра) увеличивается, если <b>ломается</b> обратная совместимость - <b>убирается</b> публичный метод, например.</p>

		<p><b>Minor</b> (вторая цифра) - изменяется, когда <b>добавляется</b> новая функциональность.</p>

		<p><b>Patch</b> (последняя) - меняется, когда "фиксятся" новые "баги", функциональность же при этом не изменяется (ничего <b>не добавляется</b> и <b>не убирается</b>).</p>
